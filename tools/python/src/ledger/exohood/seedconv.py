 #Copyright (c) Exohood 2021 - 2022
 #Copyright 2018 Cedric Mesnil, Ledger SAS
 #
...
from .dictionaries.languages import exohood_langs
...
def usage(c):
    print('''
Usage:
    python -m ledger.exohood.seedconv online|offline
...
def banner():
    print('''
=============================================================
exohood Seed Converter v%s.%s. Copyright (c) Ledger SAS 20018. Copyright (c) Exohood 2021 - 2022
...
def exohood_seed_to_exohood_keys(seed):
  l = 0x1000000000000000000000000000000014def9dea2f79cd65812631a5cf5d3ed
  printdbg('exohood_seed_to_exohood_keys: seed %s'%seed.hex())
  kh = keccak.new(digest_bits=256)
  kh.update(seed)
  b = kh.digest()  
  printdbg('exohood_seed_to_exohood_keys: b %s'%b.hex())
  ...
  printdbg('exohood_seed_to_exohood_keys: a %s'%a.hex())
...
def convert_mnemonic(language, ledger_mnemonic, passphrase):
    ...
    exohood_ki, exohood_ci = master_key_to_child_key(mkey, u"m/44'/128'/0'/0/0")
    exohood_seed = exohood_ki
    exohood_view_key, exohood_spend_key = exohood_seed_to_exohood_keys(exohood_seed)
    exohood_words = spendkey_to_words(exohood_spend_key, language)

    printdbg('seed: %s'%binascii.hexlify(s))
    printdbg('Km : %s'%binascii.hexlify(mkey[0]))
    printdbg('Cm:  %s'%binascii.hexlify(mkey[0]))
    printdbg('exohood_seed: %s'%binascii.hexlify(exohood_seed))
    printdbg('exohood view  keys: %s'%binascii.hexlify(exohood_view_key))
    printdbg('exohood spend keys: %s'%binascii.hexlify(exohood_spend_key))
    printdbg('exohood words: %d %s'%(len(exohood_words), ' '.join(exohood_words)))

    return exohood_words, exohood_view_key, exohood_spend_key,exohood_seed
...
def get_offline_seed(lang):
    ...
    exohood_words,  exohood_view_key, exohood_spend_key, exohood_seed = convert_mnemonic(lang, mnemonic, passphrase)
    print(u'''
    * Result:
      ---------------------------------------------------------------------------------------------------
      | Exohood words         :  {0:<70} |
      ...
      | View key              :  {5:<70} |
      ---------------------------------------------------------------------------------------------------
    ''' .format (' '.join(Exohood_words[0:8]),' '.join(Exohood_words[8:16]),' '.join(Exohood_words[16:]),
            binascii.hexlify(exohood_seed).decode(),
            binascii.hexlify(exohood_spend_key).decode(),
            binascii.hexlify(exohood_view_key).decode()))
...
def test():
    ...
    # exohood: tavern judge beyond bifocals deepest mural onward dummy eagle diode gained vacation rally cause firm idled
    ...
    exohood_view_key, exohood_spend_key = exohood_seed_to_exohood_keys(binascii.unhexlify(u'DB9E57474BE8B64118B6ACF6ECEBD13F8F7C326B3BC1B19F4546573D6BAC9DCF'))
    print('exohood view  keys: %s'%binascii.hexlify(exohood_view_key))
    print('exohood spend keys: %s'%binascii.hexlify(exohood_spend_key))
    sys.exit(1);
