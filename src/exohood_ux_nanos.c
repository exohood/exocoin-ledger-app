...
void ui_info(const char* msg1, const char* msg2, const void *menu_display, unsigned int value) {
  os_memset(&G_exohood_vstate.ui_dogsays[0], 0, sizeof(ux_menu_entry_t));
  G_exohood_vstate.ui_dogsays[0].callback = menu_display;
  G_exohood_vstate.ui_dogsays[0].userid   = value;
  G_exohood_vstate.ui_dogsays[0].line1    = msg1;
  G_exohood_vstate.ui_dogsays[0].line2    = msg2;

  os_memset(&G_exohood_vstate.ui_dogsays[1],0, sizeof(ux_menu_entry_t));
  UX_MENU_DISPLAY(0, G_exohood_vstate.ui_dogsays, NULL);
};
*/
/* ----------------------------- FEE VALIDATION ----------------------------- */
...
const ux_menu_entry_t ui_menu_change_validation[] = {
  {NULL,  NULL,                              1,      NULL, " Change",  "?EXO?",  0, 0},
...
const bagl_element_t* ui_menu_amount_validation_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  ...
  if ((entry == &ui_menu_fee_validation[0]) || (entry == &ui_menu_change_validation[0])) {
    if(element->component.userid==0x22) {
      element->text = G_exohood_vstate.ux_amount;
    }
  }
  return element;
}
...
#define WORDS N_exohood_pstate->words
...
const bagl_element_t* ui_menu_words_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  if ((entry->userid >= 0) && (entry->userid <25)) {
    ...
    if(element->component.userid==0x21) {
      element->text = N_exohood_pstate->words[entry->userid];
    }

    if ((element->component.userid==0x22)&&(entry->userid<24)) {
      element->text = N_exohood_pstate->words[entry->userid+1];
    }
  }
...
const ux_menu_entry_t ui_menu_validation[] = {
  {NULL,  NULL,                       1,      NULL, " Amount",       "?EXO?",      0, 0},
...
const bagl_element_t* ui_menu_validation_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  ...
  if (entry == &ui_menu_validation[1]) {
    if(element->component.userid==0x22) {
      element->text = G_exohood_vstate.ux_fees;
    }
  }
#endif
  ...
  if (entry == &ui_menu_validation[1]) {
    if(element->component.userid==0x22) {
      os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu)) ;
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*0, 11);
      element->text = G_exohood_vstate.ux_menu;
    }
  }
  if (entry == &ui_menu_validation[2]) {
    os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*1, 11);
    }
    if(element->component.userid==0x22) {
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*2, 11);
    }
    element->text = G_exohood_vstate.ux_menu;
  }
  if (entry == &ui_menu_validation[3]) {
    os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*3, 11);
    }
    if(element->component.userid==0x22) {
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*4, 11);
    }
    element->text = G_exohood_vstate.ux_menu;
  }
  if (entry == &ui_menu_validation[4]) {
    os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*5, 11);
    }
    if(element->component.userid==0x22) {
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*6, 11);
    }
    element->text = G_exohood_vstate.ux_menu;
  }
  if (entry == &ui_menu_validation[5]) {
    os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*7, 11);
    }
    if(element->component.userid==0x22) {
      os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*8, 10);
    }
    element->text = G_exohood_vstate.ux_menu;
  }
...
const bagl_element_t* ui_menu_validation_loopback_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  ...
  if (entry == &ui_menu_validation_loopback[1]) {
    if(element->component.userid==0x21) {
      element->text = G_exohood_vstate.ux_inputs;
    }
  }
...
unsigned int ui_export_viewkey_prepro(const  bagl_element_t* element) {
  if (element->component.userid == 1) {
    snprintf(G_exohood_vstate.ux_menu, sizeof(G_exohood_vstate.ux_menu), "Export");
    return 1;
  }
  if (element->component.userid == 2) {
    snprintf(G_exohood_vstate.ux_menu, sizeof(G_exohood_vstate.ux_menu), "View Key");
    return 1;
  }
  snprintf(G_exohood_vstate.ux_menu, sizeof(G_exohood_vstate.ux_menu), "Please Cancel");
  return 1;
}
...
const bagl_element_t* ui_menu_network_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu));
  if ((entry == &ui_menu_network[2]) && (element->component.userid==0x20) && (N_exohood_pstate->network_id == TESTNET)) {
    os_memmove(G_exohood_vstate.ux_menu, "Test Network  ", 14);
    G_exohood_vstate.ux_menu[13] = '+';
    element->text = G_exohood_vstate.ux_menu;
  }
  if ((entry == &ui_menu_network[3]) && (element->component.userid==0x20) && (N_exohood_pstate->network_id == STAGENET)) {
    os_memmove(G_exohood_vstate.ux_menu, "Stage Network ", 14);
    G_exohood_vstate.ux_menu[13] = '+';
    element->text = G_exohood_vstate.ux_menu;
  }
  #ifndef exohood_ALPHA
  if ((entry == &ui_menu_network[4]) && (element->component.userid==0x20) && (N_exohood_pstate->network_id == MAINNET)) {
    os_memmove(G_exohood_vstate.ux_menu, "Main Network  ", 14);
    G_exohood_vstate.ux_menu[13] = '+';
    element->text = G_exohood_vstate.ux_menu;
  }
  #endif
...
void ui_menu_reset_action(unsigned int value) {
  ...
  exohood_nvm_write(N_exohood_pstate->magic, magic, 4);
  exohood_init();
  ui_menu_main_display(0);
}
/* ------------------------------- SETTINGS UX ------------------------------- */
...
const bagl_element_t* ui_menu_pubaddr_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  ...
  if (entry == &ui_menu_pubaddr[0]) {
    ...
    if(element->component.userid==0x21) {
      ...
      case 0:
      case DISP_MAIN:
        os_memmove(G_exohood_vstate.ux_menu, "Main", 4);
        break;
      case DISP_SUB:
        os_memmove(G_exohood_vstate.ux_menu, "Sub", 3);
        break;
      case DISP_INTEGRATED:
        os_memmove(G_exohood_vstate.ux_menu, "Integrated", 10);
        break;
      }
      element->text = G_exohood_vstate.ux_menu;
    }
    if(element->component.userid==0x22) {
      os_memmove(G_exohood_vstate.ux_menu, "Address", 7);
      element->text = G_exohood_vstate.ux_menu;
    }
  }
  ...
  if (entry == &ui_menu_pubaddr[1]) {
    os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu)) ;
     if(element->component.userid==0x21) {
      ...
      case DISP_MAIN:
      case DISP_SUB:
        snprintf(G_exohood_vstate.ux_menu, sizeof(G_exohood_vstate.ux_menu), 
                 "Account: %d",
                 G_exohood_vstate.disp_addr_M);
        break;
      case DISP_INTEGRATED:
        os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.payment_id, 8);
        break;
      }
      element->text = G_exohood_vstate.ux_menu;
    }
     if(element->component.userid==0x22) {
      ...
      case DISP_SUB:
        ...
        break;
      case DISP_INTEGRATED:
        os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.payment_id+8, 8);
        break;
      }
      element->text = G_exohood_vstate.ux_menu;
    }
  ...
  if (entry == &ui_menu_pubaddr[6]) {
    os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu)) ;
    if(element->component.userid==0x21) {
       if (G_exohood_vstate.disp_addr_mode== DISP_INTEGRATED) {
        os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*8, 11);
      } else {
        os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*8, 10);
      }
    }
    if(element->component.userid==0x22) {
      if (G_exohood_vstate.disp_addr_mode== DISP_INTEGRATED) {
        os_memmove(G_exohood_vstate.ux_menu, G_exohood_vstate.ux_address+11*9, 10);
      }
    }
    element->text = G_exohood_vstate.ux_menu;
  }
...
const ux_menu_entry_t ui_menu_main[] = {
  {NULL,    ui_menu_pubaddr_display,  0, NULL,              "EXO",            "", 0, 0},
...
const bagl_element_t* ui_menu_main_preprocessor(const ux_menu_entry_t* entry, bagl_element_t* element) {
  if (entry == &ui_menu_main[0]) {
    if(element->component.userid==0x22) {
      os_memset(G_exohood_vstate.ux_menu, 0, sizeof(G_exohood_vstate.ux_menu));
      exohood_base58_public_key(G_exohood_vstate.ux_menu, G_exohood_vstate.A,G_exohood_vstate.B, 0, NULL);
      os_memset(G_exohood_vstate.ux_menu+5,'.',2);
      os_memmove(G_exohood_vstate.ux_menu+7, G_exohood_vstate.ux_menu+98-5,5);
      G_exohood_vstate.ux_menu[12] = 0;
      element->text = G_exohood_vstate.ux_menu;
    }
  }
  return element;
